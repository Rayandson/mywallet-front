...
export default function InflowPage() {
    ...
    function save(event) {
        axios.post("http://localhost:5000/transactions", {cashValue, description, type: "input", userId: user._id}, {headers: {token: token}})
        .then((res) => {
            console.log(res)
            navigate("/balance")
        }).catch ((err) => {
    ...
    return(
        ...
        <Botao type="submit" value="Salvar entrada" />
        </StyledForm>
        </Content>
        </Container>
    )
